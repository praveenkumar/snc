...
PRESET=$(${JQ} -r .type $INSTALL_DIR/crc-bundle-info.json)
destDirPrefix="crc"
if [ ${PRESET} != "snc" ]; then
    destDirPrefix="crc_${PRESET}"
    BASE_OS=rhel
fi
...
if [ "${ARCH}" == "aarch64" ] && [ ${PRESET} == "snc" ]; then
...
if [ $PRESET == "snc" ]; then
    ...
    mv "${libvirtDestDir}/${VM_NAME}" "${libvirtDestDir}/${CRC_VM_NAME}.qcow2"
else
    create_sparsify_qemu_image "${libvirtDestDir}"
fi
